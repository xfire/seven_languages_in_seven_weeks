...
%% start a node with `erl -sname translator@localhost`
%% start a second repl wit `erl -sname somename`
%% c(translate_network).
%% translate:start_translator().
%% translate:translate("casa").
%% ...

start_translator() ->
    spawn(translator@localhost, ?MODULE, translator_restarter, []).

kill_translator() ->
    translator ! diediedie.

translate(Word) ->
    {translator, translator@localhost} !{self(), Word},
    receive
        Translation -> Translation
    end.
